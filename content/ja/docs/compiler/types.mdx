...
詳細については、実際のソース [types.ts](https://github.com/yamlresume/yamlresume/blob/main/packages/core/src/models/types.ts) をご確認ください。
...
| 属性      | タイプ                              | 必須 | 説明                                          |
| --------- | --------------------------------- | -------- | ---------------------------------------------------- |
| `content` | [`ResumeContent`](#resumecontent) | ✅       | 履歴書全体のコンテンツ構造を定義します。 |
| `layout?` | [`ResumeLayout`](#resumelayout)   | ⬜       | 全体的なレイアウト設定を定義します。            |
...
| 属性          | タイプ                                                | 必須 | 説明                                                        |
| ------------- | --------------------------------------------------- | -------- | ------------------------------------------------------------------ |
| `locale?`     | [`ResumeLayoutLocale`](#resumelayoutlocale)         | ⬜       | 国際化とローカライゼーションのロケール設定を定義します。 |
| `margins?`    | [`ResumeLayoutMargins`](#resumelayoutmargins)       | ⬜       | 文書レイアウトのマージン設定を定義します。                  |
| `page?`       | [`ResumeLayoutPage`](#resumelayoutpage)             | ⬜       | 文書プレゼンテーションのページレベル設定を定義します。             |
| `template?`   | [`Template`](#template)                             | ⬜       | 選択されたテンプレートを定義します。                                     |
| `typography?` | [`ResumeLayoutTypography`](#resumelayouttypography) | ⬜       | 文書フォーマットのタイポグラフィ設定を定義します。               |
| `latex?`      | [`ResumeLayoutLaTeX`](#resumelayoutlatex)           | ⬜       | LaTeX 固有の設定オプションを定義します。                      |
...
文書フォーマットのタイポグラフィ設定を定義します。

| 属性        | タイプ     | 必須 | 説明                                             |
| ----------- | -------- | -------- | ------------------------------------------------------- |
| `fontSize?` | `string` | ⬜       | 文書の基本フォントサイズ（例：「10pt」、「11pt」）。 |

#### ResumeLayoutLaTeX

LaTeX 固有の設定オプションを定義します。

| 属性        | タイプ                            | 必須 | 説明                            |
| ----------- | ------------------------------- | -------- | -------------------------------------- |
| `fontspec?` | `{ numbers?: FontspecNumbers }` | ⬜       | LaTeX fontspec パッケージ設定。 |
...
参照：[テンプレート](https://yamlresume.dev/docs/layouts/latex/templates)
...
参照：[多言語](https://yamlresume.dev/docs/content/multi-languages)
