...
基本的に、この方法で `yamlresume` [CLI サブコマンド](/ja/docs/cli) をすべて呼び出すことができます—`yamlresume/yamlresume` の後にサブコマンドを追加し、対応する使用ガイドに従うだけです。
...
Node.js 22.x または更新バージョンが必要です。

<Callout title="なぜ Node.js 22.x または更新バージョンが必要なのですか？">
  YAMLResume パッケージがサポートする最小ランタイムは Node.js 22 です。
  それ以前の Node.js バージョンはサポートされていません。
</Callout>
...
<Tab value="Ubuntu">
Ubuntu のディストリビューションによってバージョンが異なる `nodejs` パッケージではなく、
[NodeSource](https://github.com/nodesource/distributions) の Node.js 22
リポジトリを使用してください：
...
<Tab value="RHEL">
Fedora、AlmaLinux、Rocky Linux などの RHEL ベースの Linux では、
[NodeSource](https://github.com/nodesource/distributions) の Node.js 22
リポジトリを使用してください：
