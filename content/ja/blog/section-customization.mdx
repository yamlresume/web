...
YAMLResume のデフォルトセクションタイトル（例：`education`、`work`）は、選択した [ロケール言語](/ja/docs/layouts#locale) に基づいて自動的に[翻訳](/ja/docs/layouts/sections/docs/content/multi-languages) されます。しかし、異なるタイトルを好む場合があります。例えば、「仕事」を「プロフェッショナル経験」に変更したい場合があります。
