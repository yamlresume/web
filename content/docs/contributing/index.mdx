...
<Cards>
  <Card
    ...
    description="Building and Testing CLI"
  />
  <Card
    title="Docker"
    href="./contributing/docker"
    description="Docker Customization"
  />
  <Card
    title="Test"
  ...
  <Card
    ...
    description="Add support for a new language in YAMLResume."
  />
</Cards>
...
- [Node.js](https://nodejs.org/) (version >= 22, check the `package.json`
  engines field)
...
The repository is a pnpm workspace. Build every package with:

```bash
pnpm build
```

For active development, watch all packages or target one package through its
root shortcut:

```bash
pnpm build:watch
pnpm core build:watch
pnpm node build:watch
pnpm playground build:watch
```

Build production bundles, including type declarations and source maps, with:

```bash
pnpm build:prod
```

The main package shortcuts are `ai`, `cli`, `core`, `node`, `playground`,
`samples`, `create-yamlresume`, and `json2yamlresume`. You can also use pnpm's
filter syntax directly, for example `pnpm --filter @yamlresume/node test`.
