...
YAMLResume is distributed as a Node.js CLI and a set of reusable packages. For
DOCX, HTML, and Markdown output, you only need Node.js and the `yamlresume` CLI.
Generating a PDF from a LaTeX layout additionally requires a supported
[typesetting engine](#typesetting-engine), and installing the recommended fonts
improves the final typography.

In short, a local CLI installation needs:

- Node.js 22 or newer
- the [`yamlresume` CLI package](https://www.npmjs.com/package/yamlresume)
- [optional] XeTeX or Tectonic for LaTeX/PDF output
- [optional] recommended fonts for optimal PDF output

If you prefer an environment with the CLI, XeTeX, and fonts already installed,
use the official Docker image below.
...
<Callout title="Why Node.js 22.x or newer?">
  YAMLResume packages declare Node.js 22 as their minimum supported runtime.
...
<Tab value="Ubuntu">
Use the [NodeSource](https://github.com/nodesource/distributions) Node.js 22
repository instead of Ubuntu's version-dependent `nodejs` package:
...
<Tab value="RHEL">
For RHEL-based Linux such as Fedora, AlmaLinux, and Rocky Linux, use the
[NodeSource](https://github.com/nodesource/distributions) Node.js 22 repository:
...
If you are integrating YAMLResume into a Node.js application rather than using
the command line, install the runtime package locally:

```console
$ npm install @yamlresume/node
```

`@yamlresume/node` provides file-oriented APIs for reading, validating,
building, watching, generating, and translating resumes. It uses the same
runtime implementation as the CLI. See the
[`@yamlresume/node` ecosystem guide](/docs/ecosystem/node) for setup, examples,
and links to the complete API reference.

For model-level AI APIs that operate on strings rather than files, install
