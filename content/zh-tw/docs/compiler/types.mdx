...
您可以查看 [types.ts](https://github.com/yamlresume/yamlresume/blob/main/packages/core/src/models/types.ts) 了解真實來源的詳細資訊。
...
| 屬性      | 類型                              | 必需 | 描述                                          |
| --------- | --------------------------------- | -------- | ---------------------------------------------------- |
| `content` | [`ResumeContent`](#resumecontent) | ✅       | 定義整個履歷內容的結構。 |
| `layout?` | [`ResumeLayout`](#resumelayout)   | ⬜       | 定義整體佈局配置。            |
...
| 屬性          | 類型                                                | 必需 | 描述                                                        |
| ------------- | --------------------------------------------------- | -------- | ------------------------------------------------------------------ |
| `locale?`     | [`ResumeLayoutLocale`](#resumelayoutlocale)         | ⬜       | 定義國際化和本地化的區域設置。 |
| `margins?`    | [`ResumeLayoutMargins`](#resumelayoutmargins)       | ⬜       | 定義文件佈局的頁邊距設置。                  |
| `page?`       | [`ResumeLayoutPage`](#resumelayoutpage)             | ⬜       | 定義文件演示的頁面級設置。             |
| `template?`   | [`Template`](#template)                             | ⬜       | 定義選定的模板。                                     |
| `typography?` | [`ResumeLayoutTypography`](#resumelayouttypography) | ⬜       | 定義文件格式的排版設置。               |
| `latex?`      | [`ResumeLayoutLaTeX`](#resumelayoutlatex)           | ⬜       | 定義 LaTeX 特定配置選項。                      |
...
另請參閱：[模板](https://yamlresume.dev/docs/layouts/latex/templates)
...
另請參閱：[多語言](https://yamlresume.dev/docs/content/multi-languages)
