...
自 v0.15.0 起，`yamlresume new` 會從精選的範例履歷建立您的履歷（預設為英文的
...
選項。執行 [`yamlresume samples list`](#範例履歷) 查看所有可用的範例：
...
如果給定了未知的範例 ID，CLI 會列出所有可用的範例以幫助您選擇：
...
使用 [`yamlresume new --sample <id>`](#new) 從範例建立新的履歷檔案。
