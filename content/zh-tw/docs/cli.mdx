...
YAMLResume 提供了 CLI 工具來幫助您管理履歷。您可以按照 [安裝指南](/zh-tw/docs/getting-started#installation) 來安裝它。
...
列出所有可用語言。語言代碼遵循 [ISO 639 規範](https://www.iso.org/iso-639-language-code) 中的規範。您可以在 `layout.locale.language` 中指定語言代碼以更改履歷的語言。更改履歷的語言將自動翻譯所有固定術語。您可以在我們的 [內容 -> 多語言](/zh-tw/docs/content/multi-languages) 章节查看更多詳細資訊。
...
列出所有可用模板。您可以通過在 YAML 履歷中設定 `layout.template` 來更改履歷模板。更多詳細資訊請參見 [PPResume 模板](https://docs.ppresume.com/typesetting/templates)。
