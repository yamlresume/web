...
YAMLResume 為[除 `location` 和 `profiles`](#aliasable-sections) 之外的所有履歷章節提供默認標題，這些標題會根據選擇的[語言環境語言](/zh-tw/docs/layouts#locale)自動[翻譯](docs/content/multi-languages)。但是，您可能希望自定義這些標題以更好地適合您的檔案或行業。`section aliases` 功能允許您用自己的自定義名稱覆蓋默認章節標題。
