...
基本上，您可以以這種方式調用所有 `yamlresume` [CLI 子命令](/zh-tw/docs/cli)——只需在 `yamlresume/yamlresume` 後附加子命令，然後按照相應的使用指南進行操作。
...
需要 Node.js 22.x 或更新版本。

<Callout title="為什麼需要 Node.js 22.x 或更新版本？">
  YAMLResume 套件宣告支援的最低執行環境版本為 Node.js 22。
  不支援更早的 Node.js 版本。
</Callout>
...
<Tab value="Ubuntu">
請使用 [NodeSource](https://github.com/nodesource/distributions) 的 Node.js 22
套件庫，而不是版本隨 Ubuntu 發行版變化的 `nodejs` 套件：
...
<Tab value="RHEL">
對於 Fedora、AlmaLinux 和 Rocky Linux 等基於 RHEL 的 Linux，請使用
[NodeSource](https://github.com/nodesource/distributions) 的 Node.js 22 套件庫：
