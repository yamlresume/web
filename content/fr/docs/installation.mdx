...
Node.js 22.x ou plus récent est requis.

<Callout title="Pourquoi Node.js 22.x ou plus récent ?">
  Les packages YAMLResume déclarent Node.js 22 comme version minimale prise en
  charge. Les versions antérieures de Node.js ne sont pas prises en charge.
</Callout>
...
<Tab value="Ubuntu">
Utilisez le dépôt Node.js 22 de
[NodeSource](https://github.com/nodesource/distributions) plutôt que le package
`nodejs` d'Ubuntu, dont la version dépend de la distribution :
...
<Tab value="RHEL">
Pour les distributions Linux basées sur RHEL comme Fedora, AlmaLinux et Rocky
Linux, utilisez le dépôt Node.js 22 de
...
```
MacTeX est une distribution volumineuse, donc elle peut prendre du temps à télécharger et installer et occupe environ 5 Go d'espace disque. Si vous voulez une distribution plus petite, vous pouvez installer **BasicTeX**, qui est une version minimale de MacTeX :

```sh
brew install --cask basictex
```

Après l'installation de BasicTeX, vous devrez peut-être installer des paquets supplémentaires manuellement en utilisant la commande `tlmgr` :
...
Vous pouvez consulter le blog de Kostiantyn Lysenko, [YAMLResume - Resume as Code](https://lysenko.dev/posts/2025-01-yamlresume-resume-as-code/) pour plus de détails sur basictex avec YAMLResume.

La [distribution MiKTeX](https://miktex.org) est un choix populaire pour Windows et inclut XeTeX. Vous pouvez la télécharger depuis le [site web MiKTeX](https://miktex.org/download).

Alternativement, vous pouvez installer **TeX Live** pour Windows, qui inclut aussi XeTeX. Téléchargez-le depuis le [site web TeX Live](https://www.tug.org/texlive/acquire-netinstall.html).
