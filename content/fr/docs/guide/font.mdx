...
**[Fangsong](https://en.wikipedia.org/wiki/Fangsong)**, connu sous le nom de 仿宋体 en
chinois simplifié, 仿宋體 en chinois traditionnel, est un type de police d'écriture régulière.
Il est principalement utilisé dans l'introduction, le résumé et d'autres paragraphes
...
Il y a deux types de polices inclinées dans les polices occidentales : le type oblique et le type
italique. Celui qui est incliné avec un changement dans la forme des caractères est
l'"Italique", tandis que celui qui incline simplement la police originale vers la droite
sans changement dans la forme des caractères est connu sous le nom de type oblique. Puisque
les italiques obliques, qui sont simplement inclinées et déformées par des algorithmes logiciels,
...
Il y a des tonnes de polices disponibles de nos jours, donc recommander un schéma de police occidental
...
[^3]:
    ...
    dont le principe est d'ajuster l'espacement des lettres pour
