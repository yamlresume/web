...
lettres avec une capitalisation différente peut avoir des significations complètement différentes dans
divers domaines, donc les chercheurs d'emploi doivent être très prudents. Par exemple,
[iOS](http://www.apple.com/ios/) avec un "i" minuscule représente le système d'exploitation mobile
sur un iPhone Apple, tandis qu'
[IOS](https://en.wikipedia.org/wiki/Cisco_IOS) avec un "I" majuscule est le
système dédié sur les routeurs Cisco.

Limité par nos antécédents, ce guide résume quelques erreurs d'orthographe communes de
noms propres dans le domaine de l'informatique.
