...
从 v0.15.0 开始，`yamlresume new` 会基于一份精选的示例简历来创建您的简历（默认是英文的 `software-engineer` 示例）。生成的文件带有便于编辑器/IDE 进行 schema 验证的有用 YAML 注释，以及一组默认布局，因此您可以立即构建它。

### `--sample`

如果您想从特定的精选示例简历开始，可以使用 `--sample` 选项。运行 [`yamlresume samples list`](#示例简历) 查看所有可用的示例：
...
如果给定的示例 id 不存在，CLI 会打印所有可用的示例以帮助您选择：
...
所有示例简历都翻译成了 12 种区域语言。您可以使用 `--language` 选项从本地化的示例创建简历，这对于编写非英语简历非常方便：
...
YAMLResume 为常见职位提供了一套精选的示例简历，并翻译成 12 种区域语言（`de`、`en`、`es`、`fr`、`id`、`ja`、`nl`、`no`、`pt-br`、`zh-hans`、`zh-hant-hk`、`zh-hant-tw`）。使用 `samples list` 子命令列出全部示例：
...
使用 [`yamlresume new --sample <id>`](#new) 从示例创建新的简历文件。
