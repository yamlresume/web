...
基本上，您可以以这种方式调用所有 `yamlresume` [CLI 子命令](/zh-cn/docs/cli)——只需在 `yamlresume/yamlresume` 后附加子命令，然后按照相应的使用指南进行操作。
...
<Callout title="为什么需要 Node.js 22.x 或更新版本？">
  YAMLResume 软件包声明支持的最低运行时版本为 Node.js 22。
  不支持更早的 Node.js 版本。
</Callout>
...
<Tab value="Ubuntu">
请使用 [NodeSource](https://github.com/nodesource/distributions) 的 Node.js 22
软件源，而不是版本随 Ubuntu 发行版变化的 `nodejs` 软件包：
...
<Tab value="RHEL">
对于 Fedora、AlmaLinux 和 Rocky Linux 等基于 RHEL 的 Linux，请使用
[NodeSource](https://github.com/nodesource/distributions) 的 Node.js 22 软件源：
...
```
MacTeX 是一个大型发行版，因此下载和安装可能需要一些时间，并且占用约 5GB 的磁盘空间。如果您想要一个更小的发行版，可以安装 **BasicTeX**，它是 MacTeX 的最小版本：

```sh
brew install --cask basictex
```

安装 BasicTeX 后，您可能需要使用 `tlmgr` 命令手动安装额外的包：
...
您可以查看 Kostiantyn Lysenko 的博客 [YAMLResume - Resume as Code](https://lysenko.dev/posts/2025-01-yamlresume-resume-as-code/) 了解更多关于 basictex 与 YAMLResume 的详细信息。
