...
- [章节别名](/zh-cn/docs/layout/sections/aliases)：重命名预定义章节（例如，将 `work` 改为"专业经验"，将 `education` 改为"学术背景"）使用 `layout.sections.aliases`。这对于语调、地区/文化或角色定位特别有用。
- [章节重排序](/zh-cn/docs/layout/sections/reorder)：使用 `layout.sections.order` 将最相关的章节带到顶部。将 `skills` 放在 `work` 之前，或者为作品集重的角色首先展示 `projects`。
