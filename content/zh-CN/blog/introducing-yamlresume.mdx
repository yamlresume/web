...
![YAMLResume 新建和构建简历](../../blog/static/images/introducing-yamlresume/yamlresume-new-and-build.webp)
...
![简历第 1 页](../../blog/static/images/introducing-yamlresume/resume-1.webp)
![简历第 2 页](../../blog/static/images/introducing-yamlresume/resume-2.webp)
...
虽然我们决定不使用 markdown 作为简历输入格式，但我们确实支持各个部分中 `summary` 字段的有限 markdown 富文本语法集。
...
content:
  basics:
    # ...
    summary: |
      这是一个支持有限 markdown 富文本语法集（粗体、斜体、链接、列表）的示例 YAML 简历：

      - 计算机科学专业，在数据结构、**算法**和软件开发方面有*坚实基础*
        1. 像素完美的全栈网页开发者，专门创建高质量、视觉吸引力的网站
        2. 在数据库（SQL、NoSQL）方面有经验，熟悉服务器端技术（[Node.js](https://nodejs.org/en)、Express 等）
      - 团队合作者，具有注重细节的思维和对设计和用户体验的敏锐眼光
```

这是生成的 PDF：

![摘要字段中的富文本支持](../../blog/static/images/introducing-yamlresume/rich-text-support-in-summary-field.webp)
