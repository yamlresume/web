...
describe('gallery detail messages', () => {
  it('defines complete detail labels for every site language', () => {
    for (const language of languages) {
      ...
      expect(messages.initializeExample.title).not.toBe('')
      expect(messages.initializeExample.description).not.toBe('')
      expect(messages.initializeExample.installationLink).not.toBe('')
